    ...
    with tf.Session( config = None ) as sess:
        ...
        while it() < args.epoch_size * args.n_epochs:
            ...
            if it() % args.log_step == args.log_step -1 or it() < 10 :
                ...
                step_per_epoch = int ( args.epoch_size / args.batch_size) + ( args.epoch_size % args.batch_size != 0 )
                save_images.save_images("../training_output/"+args.name,[i_hr,i_bi,i_sr],it() , int( it() / args.epoch_size )+ (it() % args.epoch_size!=0) )
...
def main(_):
    args = parse_args()
    args.name = "edsr_dim{}_{}_scale{}".format(args.dim , args.upsample_method , args.scale)
